...
public class GLFW
{
    ...
    private static long mGLFWShowingWindow = 0;
    ...
    public static long glfwCreateWindow(int width, int height, CharSequence title, long monitor, long share) {
        ...
        mainContext = ptr;
        if (mGLFWShowingWindow == 0) {
            mGLFWShowingWindow = ptr;
            nglfwSetShowingWindow(ptr);
        }
        return ptr;
        //Return our context
    }
    ...
    public static void glfwDestroyWindow(long window) {
        ...
        if (mGLFWShowingWindow == window) {
            mGLFWShowingWindow = mGLFWWindowMap.size() == 0 ? 0 : mGLFWWindowMap.keyAt(mGLFWWindowMap.size() - 1);
        }
        nglfwSetShowingWindow(mGLFWShowingWindow);
    }
    ...
    public static void glfwPollEvents() {
        long targetWindow = 0;
        if (mGLFWShowingWindow != 0 && mGLFWWindowMap.containsKey(mGLFWShowingWindow)) {
            targetWindow = mGLFWShowingWindow;
        } else if (mainContext != 0 && mGLFWWindowMap.containsKey(mainContext)) {
            targetWindow = mainContext;
        } else if (mGLFWWindowMap.size() > 0) {
            targetWindow = mGLFWWindowMap.keyAt(mGLFWWindowMap.size() - 1);
        }

        if (targetWindow != 0) {
            callJV(targetWindow, Functions.PumpEvents);
        }
