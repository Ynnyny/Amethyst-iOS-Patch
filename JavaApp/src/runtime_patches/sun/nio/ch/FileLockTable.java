...
class FileLockTable {
    ...
    private static final ReferenceQueue<FileLock> queue = new ReferenceQueue<FileLock>();

    // The channel this table belongs to.
    ...
    void add(FileLock fl) throws OverlappingFileLockException {
        ...
        for (;;) {
            // The key is not in the map, so try to create it atomically.
            if (list == null) {
            ...
            // The key already exists. It may be removed concurrently, so
            // re-fetch the list and only use it if it still matches.
            synchronized (list) {
    ...
    private void checkList(List<FileLockReference> list, long position, long size)
        ...
        for (FileLockReference ref : list) {
            FileLock fl = ref.get();
            // On iOS, Minecraft can re-open the same session.lock through a new
            // FileChannel inside the same JVM while the world is still being
            // created. Preserve the per-channel overlap guard, but let distinct
            // channels rely on the native process-level file lock instead.
            if (fl != null && fl.acquiredBy() == channel && fl.overlaps(position, size)) {
